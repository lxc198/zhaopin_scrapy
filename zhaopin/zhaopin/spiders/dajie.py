...
class dajie(RedisCrawlSpider):
    ...
    rules = (
        Rule(LinkExtractor(allow=(r'joinus'), restrict_css=('.jz-count', '.paging')), follow=True),
        Rule(LinkExtractor(allow=(r'corp\?page=\d+'), restrict_css=('.paging')), follow=True),
        Rule(LinkExtractor(allow=(r'job\.dajie\.com/.*?\.html'), restrict_css=('.cor-container')),
             callback='parse_item', process_request='pro', follow=False),
