...
class chinahr(RedisCrawlSpider):
    name = 'chinahr'
    ...
    rules = (
        Rule(LinkExtractor(allow=(r'/jobs/\d+/$',r'/jobs/\d+/[1-9]'), deny=(r'http://m\.'),
                           restrict_css=('.sideNav', '.wf-sideBar', '.pageList')), follow=True),
    ...
    def parse_item(self, response):
        ...
        content = html.find('div',{'class':'job_intro_info'}).text.replace('\xa0','').strip()
        p = re.compile(r'(\d+)')
        exp = p.search(exp)
        if not exp:
            exp = 0
